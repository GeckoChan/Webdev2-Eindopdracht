...
class Achievementrepository extends Repository
{
    ...
    {
        try {
            $query = "SELECT achievement_id, title, description, image_path, progress_current, progress_limit FROM achievements";
    ...
    {
        try {
            $stmt = $this->connection->prepare("SELECT achievement_id, title, description, image_path, progress_current, progress_limit FROM achievements WHERE achievement_id = :achievement_id");
    ...
    {
        try {
            $stmt = $this->connection->prepare("INSERT INTO achievements (title, description, image_path, progress_current, progress_limit) VALUES (:title, :description, :image_path, :progress_current, :progress_limit)");
            ...
            $stmt->bindParam(':progress_current', $achievement->progress_current);
